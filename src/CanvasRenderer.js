...
export class CanvasRenderer {
  ...
  _renderLineInline(ctx, line) {
    ...
    // Support both lineType:'orthogonal' (new) and routing:'orthogonal' (legacy)
    const isOrthogonal = line.lineType === 'orthogonal' || line.routing === 'orthogonal';
    ...
    // ─── Orthogonal ────────────────────────
    if (isOrthogonal) {
      // Use stored waypoints if available (user-edited), otherwise auto-calculate
      const segs = (line.lineType === 'orthogonal' && Array.isArray(line.waypoints) && line.waypoints.length >= 2)
        ? line.waypoints
        : this._calcOrthogonalPath(line);
      ...
      if (line.arrowTail && line.arrowTail !== 'none') {
        ...
        this._drawArrowHead(ctx, first.x, first.y, Math.atan2(first.y - second.y, first.x - second.x), line.arrowTail, stroke, strokeWidth);
      }
      this._renderConnectionDots(ctx, line);
      if (line.label?.text) {
        const midIdx = Math.floor(segs.length / 2);
        const lx = (segs[midIdx - 1].x + segs[midIdx].x) / 2;
        const ly = (segs[midIdx - 1].y + segs[midIdx].y) / 2;
