 ...
 */
interface UseFATProtocolParams {
  nodes: PLMNode[];
  edges: PLMEdge[];
}

export default function useFATProtocol({ nodes, edges }: UseFATProtocolParams) {
  const objectName = useStore(s => s.objectName);
  const objectVersion = useStore(s => s.objectVersion);
const generateFATProtocol = useCallback(async (testCaseNode: PLMNode) => {
  ...
  const linkedReqs = edges
    ...
    .filter((n: any) => n && !['testcase', 'testrun', 'testresult'].includes(n.data.itemType));
