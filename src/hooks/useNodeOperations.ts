...
/**
 * Batch operations on selected nodes: align, lock, export, group, ungroup, auto-layout.
 */
...
export default function useNodeOperations({
  ...
  const alignSelectedNodes = useCallback((direction: string) => {
    ...
    const positions = selectedNodes.map((n: any) => n.position);
    let targetValue;

    switch (direction) {
      case 'left':
        targetValue = Math.min(...positions.map(p => p.x));
        setNodes(nds => nds.map((n: any) => n.selected ? { ...n, position: { ...n.position, x: targetValue } } : n));
        break;
      case 'right':
      ...
      case 'top':
        ...
        setNodes(nds => nds.map((n: any) => n.selected ? { ...n, position: { ...n.position, y: targetValue } } : n));
        break;
      case 'bottom':
      ...
      case 'distributeH': {
        ...
        setNodes(nds => nds.map((n: any) => n.selected ? { ...n, position: { ...n.position, x: xMapping[n.id] } } : n));
        break;
      }
      case 'distributeV': {
        ...
        setNodes(nds => nds.map((n: any) => n.selected ? { ...n, position: { ...n.position, y: yMapping[n.id] } } : n));
        break;
      }
      default:
        break;
  ...
  const groupSelectedNodes = useCallback(() => {
    ...
    const groupNode = {
      ...
      data: {
        ...
        groupColor: '#3498db',
        childIds: selectedNodes.map((n: any) => n.id),
        description: `Group containing ${selectedNodes.length} items`,
        onChange: handleNodeLabelChange
      }
    };

    setNodes(nds => [groupNode, ...nds.map((n: any) => ({ ...n, selected: false }))]);
  ...
  const ungroupSelectedNodes = useCallback(() => {
    const selectedGroups = nodes.filter((n: any) => n.selected && n.data?.isGroup);
    if (selectedGroups.length === 0) return;

    const groupIds = selectedGroups.map(g => g.id);
    setNodes(nds => nds.filter((n: any) => !groupIds.includes(n.id)));
