...
/**
 * Manages all item ID counters (SYS, SUB, FUN, TC, PAR, HW, UC, ACT, CUS, PLT, PRJ, IMP)
 * and provides generateItemId() to mint new IDs.
 */
export default function useIdCounters() {
  ...
  const generateItemId = useCallback((itemType) => {
    ...
    switch (itemType) {
      case 'system': {
        const id = `SYS-${pad(sysIdCounter)}`;
        setSysIdCounter(c => c + 1);
        return id;
      }
      case 'subsystem': {
        const id = `SUB-${pad(subIdCounter)}`;
        setSubIdCounter(c => c + 1);
        return id;
      }
      case 'function': {
        const id = `FUN-${pad(funIdCounter)}`;
        setFunIdCounter(c => c + 1);
        return id;
      }
      case 'testcase': {
        const id = `TC-${pad(tcIdCounter)}`;
        setTcIdCounter(c => c + 1);
        return id;
      }
      case 'usecase': {
        const id = `UC-${pad(ucIdCounter)}`;
        setUcIdCounter(c => c + 1);
        return id;
      }
      case 'actor': {
        const id = `ACT-${pad(actIdCounter)}`;
        setActIdCounter(c => c + 1);
        return id;
      }
      case 'customer': {
        const id = `CUS-${pad(cusIdCounter)}`;
        setCusIdCounter(c => c + 1);
        return id;
      }
      case 'platform': {
        const id = `PLT-${pad(pltIdCounter)}`;
        setPltIdCounter(c => c + 1);
        return id;
      }
      case 'implementation': {
        const id = `IMP-${pad(impIdCounter)}`;
        setImpIdCounter(c => c + 1);
        return id;
      }
      case 'parameter': {
        const id = `PAR-${pad(parIdCounter)}`;
        setParIdCounter(c => c + 1);
        return id;
      }
      case 'hardware': {
        const id = `HW-${pad(hwIdCounter)}`;
        setHwIdCounter(c => c + 1);
        return id;
      }
      default: {
        const id = `PRJ-${pad(prjIdCounter)}`;
  ...
  /** Set all counters from loaded project nodes */
  const setCountersFromNodes = useCallback((projectNodes) => {
    if (!projectNodes || projectNodes.length === 0) {
      // Reset for empty project
      setNodeId(1);
    ...
    const maxNodeId = Math.max(...projectNodes.map(n => parseInt(n.id) || 0), 0);
    ...
    console.log('Updated counters - nodeId:', maxNodeId + 1, 'SYS:', maxSys + 1, 'HW:', maxHw + 1);
  }, []);

  /** Increment a specific counter by type (used by paste/duplicate) */
  const incrementCounter = useCallback((itemType) => {
    switch (itemType) {
  ...
  return {
    ...
    setNodeId,
    generateItemId,
    setCountersFromNodes,
    incrementCounter,
    // Expose raw counters for paste/duplicate which reads current values
    counters: {
