...
export default function SequenceProperties({
  ...
  if (!element || !elementType) {
    return (
      <div style={{
        ...
        background: '#111827', borderTop: '1px solid #1e293b', borderLeft: '1px solid #1e293b',
  ...
  const labelStyle: React.CSSProperties = {
    color: '#475569', fontSize: 9, fontWeight: 700, textTransform: 'uppercase',
    letterSpacing: 0.8, marginBottom: 2,
  };

  const deleteBtn = (onClick: () => void) => (
    <button onClick={onClick} style={{
      padding: '4px 10px', background: '#7f1d1d', border: '1px solid #ef444440',
      borderRadius: 4, color: '#ef4444', fontSize: 10, cursor: 'pointer', marginTop: 8,
    }}>
      🗑️ Ta bort
    </button>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: '10px 14px', borderBottom: '1px solid #1e293b', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <span style={{ color: '#475569', fontSize: 9, fontFamily: 'monospace' }}>{element.id?.slice(-8)}</span>
      </div>
                ...
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {Object.entries(PARTICIPANT_COLORS).map(([key, color]) => (
                    <button
                      ...
                      style={{
                        width: 22, height: 22, borderRadius: 4, border: p.color === color ? '2px solid white' : '1px solid #334155',
                  ...
                  {(['sync','async','reply','create','destroy'] as MessageType[]).map(t => (
                    <button key={t} onClick={() => onUpdateMessage(m.id, { type: t })}
                      style={{
                        ...
                        background: m.type === t ? '#3b82f620' : 'transparent',
                        border: m.type === t ? '1px solid #3b82f640' : '1px solid #334155',
                        color: m.type === t ? '#93c5fd' : '#94a3b8', fontSize: 10, fontFamily: 'monospace',
                      }}>{t}</button>
                  ))}
                </div>
              </div>
                ...
                <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
                  {(['alt','opt','loop','par','break','critical','ref'] as FragmentType[]).map(ft => (
                    <button key={ft} onClick={() => onUpdateFragment(f.id, { type: ft })}
                      style={{
                        padding: '2px 6px', borderRadius: 3, cursor: 'pointer',
                        background: f.type === ft ? '#f59e0b20' : 'transparent',
                        border: f.type === ft ? '1px solid #f59e0b40' : '1px solid #334155',
                        color: f.type === ft ? '#fbbf24' : '#94a3b8', fontSize: 10, fontFamily: 'monospace',
                      }}>{ft}</button>
                  ))}
                </div>
              </div>
