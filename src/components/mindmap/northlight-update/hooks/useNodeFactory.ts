...
export default function useNodeFactory({
  ...
  const addSWComponentNode = useCallback((
    familyKey: string,
    variantKey: string,
    dropPosition?: { x: number; y: number },
    naming?: { instanceName: string; posNo: string; label: string; systemCode?: string },
  ) => {
    ...
    // Use naming engine values if provided, otherwise fallback
    const instanceName = naming?.instanceName || `fb_${family.name.toLowerCase()}${variantKey}${String(Date.now()).slice(-4)}`;
    const posNo = naming?.posNo || '';
    const systemCode = naming?.systemCode || '';
    const nodeLabel = naming?.label || `${family.icon} ${familyKey} (${variantKey})`;
